...
if (isset($_POST['simpan_lisensi'])) {
    ...
    if (!empty($_FILES['file_lisensi']['name'])) {
        $filename = $_FILES['file_lisensi']['name'];
        $tmp_name = $_FILES['file_lisensi']['tmp_name'];
        $ext      = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
        $new_name = time() . '_lisensi_wasit_' . $id_user . '.' . $ext;
        $folder   = '../uploads/lisensi/';
        ...
        if (in_array($ext, ['pdf', 'jpg', 'jpeg', 'png'])) {
            if (move_uploaded_file($tmp_name, $folder . $new_name)) {
                ...
                // Hapus file lama
                $q_old = mysqli_query($conn, "SELECT file_lisensi FROM wasit WHERE id_wasit='$id_user'");
                $old_data = mysqli_fetch_assoc($q_old);
                if (!empty($old_data['file_lisensi']) && file_exists($folder . $old_data['file_lisensi'])) {
                    unlink($folder . $old_data['file_lisensi']);
                }

                // Update DB
                $sql = "UPDATE wasit SET 
                        lisensi_grade = '$grade', 
                        file_lisensi = '$new_name', 
        ...
        // Cuma Ganti Grade
        $sql = "UPDATE wasit SET lisensi_grade = '$grade', status_verifikasi = 'pending' WHERE id_wasit = '$id_user'";
        if (mysqli_query($conn, $sql)) {
...
// --- AMBIL DATA TERBARU ---
$data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM wasit WHERE id_wasit = '$id_user'"));

// Helper Vars
$file_ada = !empty($data['file_lisensi']);
$status   = $data['status_verifikasi'];

$title = "Manajemen Lisensi Wasit";
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-500">Kelola riwayat lisensi perwasitan Anda.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-8">
        <div class="p-6 border-b border-gray-50 flex justify-between items-center bg-gray-50">
            <h3 class="font-bold text-gray-800 flex items-center gap-2">
                <i class="fa-solid fa-list-ul text-yellow-600"></i> Data Lisensi Saat Ini
            </h3>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-yellow-50/30 transition">
                        <td class="px-6 py-4">
                            ...
                            <div class="text-xs text-gray-400 mt-0.5">Lisensi Wasit</div>
                        </td>
                        <td class="px-6 py-4">
                            <?php if($status == 'verified'): ?>
                        ...
                        <td class="px-6 py-4">
                            <a href="../uploads/lisensi/<?= $data['file_lisensi'] ?>" target="_blank" class="inline-flex items-center gap-2 bg-white border border-gray-200 hover:border-yellow-500 hover:text-yellow-600 px-4 py-2 rounded-lg text-xs font-bold text-gray-600 transition shadow-sm">
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="relative">
                        <select name="grade" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-yellow-500 outline-none bg-white transition">
                ...
                <div>
                    ...
                    <input type="file" name="file_lisensi" accept=".pdf,.jpg,.jpeg,.png" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100 transition border border-gray-200 rounded-xl cursor-pointer">
                ...
                <div class="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-4">
                    <div class="flex gap-3">
                        ...
                        <div class="text-xs text-amber-800 leading-relaxed">
                            ...
                            Mengupload lisensi baru akan mengubah status akun Anda menjadi <b>PENDING</b> sementara waktu hingga Admin selesai memverifikasi dokumen tersebut.
                        </div>
                    </div>
                </div>

                <button type="submit" name="simpan_lisensi" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-yellow-200 transition flex justify-center items-center gap-2">
                    <i class="fa-solid fa-paper-plane"></i> 
                    <?= $file_ada ? 'Simpan & Ajukan Ulang' : 'Kirim Lisensi' ?>
                </button>
            </div>
